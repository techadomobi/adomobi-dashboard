...
const SkeletonLoader = () => (
  ...
  const StatCard = ({ title, value, currency, isPercentage, subMetrics, isLoading }) => {
    ...
    return (
      <div className="bg-gradient-to-br from-white to-gray-50 border-2 border-blue-200 rounded-2xl shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 ease-in-out overflow-hidden">
          ...
          {isLoading ? (
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-3">
                ...
                <ArrowUpRight className="text-blue-600" size={22} />
              </div>
              <div className="flex items-baseline gap-2 mb-5">
                <h2 className="text-5xl font-extrabold text-blue-700">{formattedMainValue()}</h2>
                {isPercentage && <span className="text-4xl font-bold text-blue-700">%</span>}
              </div>
...
function MainContent() {
  ...
  const cardStructure = useMemo(() => [
    ...
    { title: "Spends", value: 0, currency: "USD", subMetrics: [ { label: "Yesterday", value: 0 }, { label: "This Week", value: 0 }, { label: "MTD", value: 0 } ] },
  ], []);
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        if (!advertiserDataString) return;
        ...
        const todayOnlyMetricsConfig = {
          ...
          // --- THIS IS THE FIX ---
          // The parser now correctly extracts the numeric value from the nested object.
          Spends: { 
            endpoint: "totalAdvertiserRevenue", 
            parser: (data) => data?.responseResult?.['$'] ? parseFloat(data.responseResult['$']) : 0 
          },
        };
        ...
        promises = promises.concat(
            Object.entries(multiPeriodMetricsConfig).flatMap(([metricName, config]) => 
                periods.map(async (period) => {
                    ...
                    const url = `${API_BASE}advertiser/${config.endpoint}?partners_Id=${partners_Id}&advertiser_id=${advertiserId}&startDate=${startDate}&endDate=${endDate}`;
                    try {
                        ...
                        if (!response.ok) return { metricName, period, value: 0 };
        ...
        promises = promises.concat(
            Object.entries(todayOnlyMetricsConfig).map(async ([metricName, config]) => {
                const url = `${API_BASE}advertiser/${config.endpoint}?partners_Id=${partners_Id}&advertiserId=${advertiserId}`;
                try {
                    const response = await fetch(url, {
                      headers: {
                        'Authorization': `Bearer ${token}`,
                    ...
                    if (!response.ok) return { metricName, period: 'today', value: 0 };
                    const data = await response.json();
                    const value = config.parser(data);
  ...
  return (
    <main className="min-h-screen w-full bg-gradient-to-br from-blue-50 to-gray-50 p-4 sm:p-6 lg:p-8">
      <style jsx global>{`
        @keyframes fadeIn {
