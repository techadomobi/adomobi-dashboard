...
const getPerformanceBadge = (clicks) => {
  ...
  return {
    variant: "info",
    label: "Growing",
    color: "bg-gradient-to-r from-blue-500 to-cyan-600 text-white shadow-sm shadow-blue-500/25",
...
export default function EnhancedAnalyticsDashboard() {
  ...
  useEffect(() => {
    setIsMounted(true)
    try {
      const storedDataString = localStorage.getItem("advertiserData")
      if (storedDataString) {
        const storedData = JSON.parse(storedDataString)
        // Support both partners_Id and _id
        const partnersId = storedData.partners_Id || storedData._id;
        // Support both advertiserId and publisherId
        const advId = storedData.advertiserId || storedData.publisherId;
        
        if (partnersId) setPartnerId(String(partnersId))
        if (advId) setAdvertiserId(String(advId))
      } else {
        console.warn("advertiseData not found in localStorage. Cannot fetch data.")
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const generalApiUrl = new URL(`${API_BASE}report/advertiserPerformanceReport`)
      generalApiUrl.searchParams.append("partners_Id", partnerId)
      generalApiUrl.searchParams.append("advertiser_id", advertiserId)
      if (startDate) generalApiUrl.searchParams.append("startDate", startDate)
      if (endDate) generalApiUrl.searchParams.append("endDate", endDate)
      ...
      if (!generalResponse.ok) {
        throw new Error(`General Report API Error: ${generalResponse.status} ${generalResponse.statusText}`)
      }
      const generalJson = await generalResponse.json()
      setGeneralReportData(generalJson.responseResult || [])

      // Fetch Event Report Data
      const eventApiUrl = new URL(`${API_BASE}eventReport/advertiserEventValueReport`)
      eventApiUrl.searchParams.append("partners_Id", partnerId)
      eventApiUrl.searchParams.append("advertiserId", advertiserId)

      const eventResponse = await fetch(eventApiUrl.toString(), { 
        headers: { 
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
        },
      ...
      if (!eventResponse.ok) {
        throw new Error(`Event Report API Error: ${eventResponse.status} ${eventResponse.statusText}`)
      }
      ...
      <div className="container mx-auto p-2 space-y-2">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-2">
          <div className="space-y-0.5">
            <h1 className="text-2xl font-black bg-gradient-to-r from-slate-900 via-blue-900 to-indigo-900 bg-clip-text text-transparent tracking-tight">
              Analytics Hub
            </h1>
        ...
        <Card className="bg-white/80 backdrop-blur-sm shadow-md shadow-blue-500/5 border-white/20 rounded-lg overflow-hidden p-3">
                ...
                <Popover open={isDatePopoverOpen} onOpenChange={setIsDatePopoverOpen}>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      variant="outline"
                      className={cn(
                        "w-full sm:w-56 justify-start text-left font-medium h-9 px-3 rounded-md border border-slate-200 bg-white hover:bg-slate-50 hover:border-blue-300 transition-all duration-200 shadow-sm text-sm",
                    ...
                    <div className="flex flex-wrap p-1.5 gap-1 border-t border-slate-100 bg-slate-50/50">
                      {getDatePresetsList().map((preset, index) => (
                        <Button
                          ...
                          className="text-xs text-slate-700 hover:bg-blue-100 hover:text-blue-700 rounded-md font-medium px-2 py-0.5"
                        >
                          {preset.label}
                        </Button>
                      ))}
        ...
        {isLoading && !generalReportData.length ? (
          ...
          <Tabs defaultValue="generalReport" className="w-full space-y-4">
            <TabsList className="grid w-full grid-cols-2 lg:w-auto lg:inline-flex bg-white/80 backdrop-blur-sm p-1 rounded-lg shadow-md shadow-blue-500/5 border border-white/20">
              <TabsTrigger
                value="generalReport"
                ...
                data-[state=active]:text-white data-[state=active]:shadow-md data-[state=active]:shadow-blue-500/10 data-[state=active]:scale-[1.01]"
              >
            ...
            <TabsContent value="generalReport" className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-3">
                <Card className="bg-gradient-to-br from-blue-500 to-indigo-600 text-white shadow-lg shadow-blue-500/10 border-0 rounded-xl overflow-hidden group hover:scale-[1.01] transition-all duration-200 p-3">
              ...
              <Card className="bg-white/80 backdrop-blur-sm shadow-lg shadow-blue-500/5 border-white/20 rounded-xl overflow-hidden">
                <CardHeader className="p-3 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-blue-50">
                  <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center justify-between">
                    ...
                    <div className="relative w-full sm:w-56">
                      ...
                      <Input
                        ...
                        className="pl-8 pr-2.5 py-1.5 h-8 rounded-md border border-slate-200 focus:ring-1 focus:ring-blue-200 focus:border-blue-400 bg-white shadow-sm text-xs"
                      />
                    </div>
                  </div>
                </CardHeader>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                      {generalPaginatedData.map((offer, index) => {
                        ...
                        return (
                          <Card
                            key={offer.offerId}
                            className="group relative bg-white shadow-sm hover:shadow-md transition-all duration-150 border  rounded-lg overflow-hidden hover:scale-[1.01] border-blue-400"
                          >
                            ...
                            <CardContent className="p-3 space-y-3 relative z-10">
                              ...
                              <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className="text-xl font-black text-blue-700">{(offer.clicks || 0).toLocaleString()}</span>
                                </div>
                                <div className="space-y-0.5">
                ...
                {generalTotalPages > 1 && (
                  <CardFooter className="justify-center pt-5 pb-3 border-t border-slate-100 bg-gradient-to-r from-slate-50 to-blue-50">
                    <Pagination>
                      <PaginationContent>
                        <PaginationPrevious href="#" onClick={(e) => { e.preventDefault(); setGeneralCurrentPage((p) => Math.max(1, p - 1))}} className="hover:bg-blue-100 rounded-md h-7 w-7 text-xs" />
                        {[...Array(generalTotalPages)].map((_, i) => {
                          ...
                          if (pageNum === 1 || pageNum === generalTotalPages || (pageNum >= generalCurrentPage - 1 && pageNum <= generalCurrentPage + 1)) {
                            return (<PaginationItem key={pageNum}><PaginationLink href="#" isActive={pageNum === generalCurrentPage} onClick={(e) => { e.preventDefault(); setGeneralCurrentPage(pageNum)}} className={cn("hover:bg-blue-100 rounded-md font-bold h-7 w-7 text-xs", pageNum === generalCurrentPage && "bg-gradient-to-r from-blue-500 to-indigo-600 text-white hover:from-blue-600 hover:to-indigo-700 shadow-sm")}>{pageNum}</PaginationLink></PaginationItem>)
                          }
                        ...
                        <PaginationNext href="#" onClick={(e) => { e.preventDefault(); setGeneralCurrentPage((p) => Math.min(generalTotalPages, p + 1))}} className="hover:bg-blue-100 rounded-md h-7 w-7 text-xs"/>
                      </PaginationContent>
                    </Pagination>
                  </CardFooter>
                )}
            ...
            <TabsContent value="eventReport" className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                <Card className="bg-gradient-to-br from-cyan-500 to-blue-600 text-white shadow-lg shadow-cyan-500/10 border-0 rounded-xl overflow-hidden group hover:scale-[1.01] transition-all duration-200 p-3">
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1">
                  ...
                  <CardContent className="p-0 pt-1">
                    ...
                    <p className="text-cyan-100 text-xs font-medium">Views</p>
                  </CardContent>
                </Card>
                <Card className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-lg shadow-emerald-500/10 border-0 rounded-xl overflow-hidden group hover:scale-[1.01] transition-all duration-200 p-3">
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1">
                      ...
                      <TableBody className="divide-y divide-slate-100">
                        {eventPaginatedData.length === 0 ? (
                          ...
                          eventPaginatedData.map((event, index) => (
                            <TableRow key={event.event_id} className="hover:bg-gradient-to-r hover:from-emerald-50 hover:to-cyan-50 transition-all duration-150">
                              ...
                              <TableCell className="py-2 px-3"><Badge className="bg-gradient-to-r from-blue-100 to-cyan-100 text-blue-800 px-2.5 py-0.5 rounded-full text-xs font-bold border border-blue-200">{event.eventValues || "N/A"}</Badge></TableCell>
