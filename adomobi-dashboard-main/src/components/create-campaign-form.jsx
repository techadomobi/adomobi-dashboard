...
export default function OfferCreateFormForAdvertiser() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const LoggedInUser = JSON.parse(localStorage.getItem("advertiserData"))
      if (LoggedInUser?._id) {
        formData.append("partners_Id", LoggedInUser._id)
      } else {
      ...
      if (LoggedInUser?.advertiserId) {
        // Don't append again if already in form data
        if (!data.advertiserId) {
          formData.append("advertiserId", LoggedInUser.advertiserId)
        }
      }
  ...
  return (
    <div className="min-h-screen py-4 px-2 sm:px-6 lg:px-2">
      <Card className="w-full max-w-8xl mx-auto rounded-2xl overflow-hidden bg-white shadow-xl border border-gray-200">
        <CardHeader className="space-y-1 p-6 bg-blue-100 text-white">
          <CardTitle className="text-3xl text-blue-900 font-bold">Create New Offer</CardTitle>
        ...
        <CardContent className="p-6 lg:p-10">
          {formSuccess && (
            <div className="mb-6 p-4 bg-blue-100 text-blue-700 rounded-md">Offer created successfully!</div>
          )}
