...
// const BASE_URL = "https://apiv2.offersmeta.in/"
...
export default function FullHeightLoginPage() {
      ...
      if (currentDomain === "localhost" || currentDomain === "127.0.0.1") {
        ...
        const fetchDomainData = async () => {
          try {
            ...
            if (data && data.partners_Id) {
              setPartnersId(data.partners_Id)
              localStorage.setItem("partnerDomainId", data._id || "")
            }
          } catch (e) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.responseCode === 200 && data.responsResult) {
        const userObject = data.responsResult
        ...
        switch (userObject.userType) {
          ...
          case "PUBLICHER":
            ...
            break
          case "MANAGER":
            if (userObject.managerRole === "Advertiser Manager") {
              localStorage.setItem("advertisermanagerData", JSON.stringify(userObject))
              localStorage.setItem("advertisermanagerToken", token)
              router.push("/advertiser-manager")
            } else {
              localStorage.setItem("managerData", JSON.stringify(userObject))
  ...
  return (
    <main className="relative w-full min-h-screen overflow-hidden bg-gradient-to-br from-white via-teal-50 to-cyan-50">
      {/* --- START: Enhanced Background Animations --- */}
      <div className="absolute inset-0 z-0 overflow-hidden">
        {/* --- Enhanced Aurora Background --- */}
        <div className="aurora-blob w-[800px] h-[800px] bg-teal-200/25 animation-aurora-move"></div>
        <div className="aurora-blob w-[700px] h-[700px] bg-cyan-200/20 animation-delay-2000 animation-aurora-float"></div>
        <div className="aurora-blob w-[900px] h-[900px] bg-emerald-200/15 animation-delay-4000 animation-aurora-move"></div>
        <div className="aurora-blob w-[600px] h-[600px] bg-purple-200/15 animation-delay-6000 animation-aurora-float"></div>

        {/* --- Highlighted Floating Geometric Shapes --- */}
        <div className="absolute inset-0 z-[1]">
          {/* Note the increased opacity in the bg color classes */}
          <div className="shape shape-1 bg-teal-200/20"></div>
          <div className="shape shape-2 bg-cyan-200/20"></div>
          <div className="shape shape-3 bg-emerald-200/15"></div>
          <div className="shape shape-4 bg-purple-200/15"></div>
        </div>
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 w-full min-h-screen">
        ...
        <div className="flex flex-col justify-between p-12">
          <div>
            <div className="flex items-center gap-3 mb-16 animate-fade-in-left">
              <Image src="/Offersmeta1.png" alt="OffersMeta" width={200} height={40} />
            </div>

            <h1 className="text-5xl xl:text-6xl font-bold text-teal-900 leading-tight mb-6 animate-fade-in-left">
            ...
            <p className="text-xl lg:text-2xl text-teal-700 mb-12 animate-fade-in-left">
              Harness the power of AI to optimize your campaigns and achieve unparalleled ROI.
            </p>

            <div className="space-y-2">
              {[
                { icon: Zap, text: "Lightning-fast global delivery", color: "text-teal-600" },
                { icon: Shield, text: "Bank-level security & fraud protection", color: "text-cyan-600" },
                ...
                >
                  ...
                  <span className="text-teal-800 text-lg">{feature.text}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex gap-12 mt-4">
            <div>
              <p className="text-3xl lg:text-4xl font-bold text-teal-900">50K+</p>
              <p className="text-teal-700 lg:text-lg">Active Users</p>
            </div>
            <div>
              <p className="text-3xl lg:text-4xl font-bold text-teal-900">99.9%</p>
              <p className="text-teal-700 lg:text-lg">Uptime</p>
            </div>
            <div>
              <p className="text-3xl lg:text-4xl font-bold text-teal-900">24/7</p>
              <p className="text-teal-700 lg:text-lg">Support</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col justify-center items-center p-6 lg:p-8 bg-white/30 backdrop-blur-sm">
          <div className="w-full max-w-lg">
            ...
            <div className="w-full bg-white/70 backdrop-blur-md border border-teal-200/50 rounded-2xl p-8 shadow-lg shadow-teal-100/50">
              <div className="text-center mb-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="h-12 pl-11 bg-teal-50/50 border-2 border-teal-200/60 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/30 rounded-lg text-slate-900 placeholder:text-slate-400 transition-colors text-base"
                      required
                    />
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Link
                      ...
                      className="text-sm lg:text-base text-teal-600 hover:text-teal-700 font-medium transition-colors"
                    >
                      Forgot password?
                    </Link>
                  </div>
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="h-12 pl-11 pr-12 bg-teal-50/50 border-2 border-teal-200/60 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/30 rounded-lg text-slate-900 placeholder:text-slate-400 transition-colors text-base"
                      required
                    />
                    <Button
                      type="button"
                      variant="ghost"
                      size="icon"
                      className="absolute right-2 top-1/2 -translate-y-1/2 h-8 w-8 hover:bg-teal-100 rounded-lg"
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="remember"
                    className="focus:ring-teal-500 border-teal-300 data-[state=checked]:bg-teal-600 data-[state=checked]:border-teal-500 rounded h-4 w-4"
                  />
                ...
                <Button
                  type="submit"
                  disabled={isLoading}
                  className="w-full h-12 text-base bg-gradient-to-r from-teal-600 to-cyan-500 hover:brightness-110 text-white font-semibold rounded-lg shadow-md hover:shadow-teal-500/30 transition-all duration-300 transform hover:scale-105 disabled:opacity-70 group"
                >
                  {isLoading ? (
              ...
              <p className="text-center text-sm lg:text-base text-slate-600 mt-6 pt-6 border-t border-teal-200/50">
                New to OffersMeta?{" "}
                <Link
                  href="/signup"
